...
export class AliasQueries extends BaseQueries<item.Alias, item.CreateAlias, item.UpdateAlias> {
  constructor(db: Database.Database) {
    super(db, 'aliases', COLUMNS, SORT_ORDER);
  }
  ...
  getByItemId(itemId: string): item.Alias[] | null {
    return this.db
      .prepare(`
        SELECT
          ${this.columns}
        ...
        ORDER BY
          ${this.tableOrder}
      `)
      .all({ itemId }) as item.Alias[];
  }
  getByIds(itemId: string, alias: string): item.Alias | null {
    return this.db
      .prepare(`
        SELECT ${this.columns}
        FROM ${this.tableName}
        WHERE item_id = ? AND alias = ?
      `)
      .get(itemId, alias) as item.Alias | null;
  }
}
